...
import { getBranches, getBranchesCount } from "@/actions/branchActions";
...
export default async function BranchesPage({ searchParams }) {
  ...
  const [branches, totalBranches] = await Promise.all([
    getBranches({ skip: (currentPage - 1) * 10, limit: 10 }),
    getBranchesCount(),
  ]);
  ...
  const renderRow = (item) => (
    <tr key={item._id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight">
      ...
      <td>{item.address?.address || "No Address"}</td>
      <td>
        <div className="flex items-center gap-2">
          <FormModal table="Branches" type="update" data={JSON.stringify(item)} />
          <FormModal table="Branches" type="delete" id={item._id.toString()} />
        </div>
      </td>
    </tr>
  );
  ...
  return (
    <div className='bg-white p-4 rounded-md m-4 mt-0 flex-1'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex flex-col md:flex-row items-center gap-4 w-full md:w-auto'>
          ...
          <div className='flex items-center gap-4 self-end'>
            ...
            <FormModal table="Branches" type="create" />
          </div>
        </div>
      </div>
      {/* List */}
      ...
      {/* Pagination */}
      <Pagination page={currentPage} count={totalBranches} />
    </div>
  );
}
