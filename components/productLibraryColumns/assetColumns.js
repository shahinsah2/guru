// @/components/productLibraryColumns/assetColumns.js
...
const checkPermissions = (roles, moduleName, permissionKey) => {
  for (const role of roles) {
    const module = role.module_access?.find((mod) => mod.module_name === moduleName);
        ...
        <>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {canDelete && <DropdownMenuItem onClick={() => setIsDeleteConfirmOpen(true)}>Delete</DropdownMenuItem>}
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          {isDeleteConfirmOpen && (
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
              <div className="bg-white p-6 rounded-md max-w-sm mx-auto">
                ...
                <p className="mt-2 text-sm">Are you sure you want to delete this asset?</p>
...
export const CreateNewAssetButton = () => {
  ...
  return (
    <div className="flex justify-end mb-1">
      <Button className="bg-blue-500 text-white" onClick={() => router.push("/product-library/assets/new")}>
        Create New Asset
      </Button>
    </div>
  );
